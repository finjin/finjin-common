...
FINJIN_MAGIC_BYTES = bytes('finj', 'utf-8')
FINJIN_MAGIC = struct.unpack('I', process_bytes(FINJIN_MAGIC_BYTES, False))[0]
FINJIN_MAGIC_REVERSED = struct.unpack('I', process_bytes(FINJIN_MAGIC_BYTES, True))[0]
...
def get_file_format_class(ext):
    if ext in ['bmp', 'dxt', 'jpg', 'jpeg', 'png', 'pvr', 'pkm', 'ktx', 'tga']:
...
def wrap_embedded_file(in_file_path, swap_bytes, out_file_path):
    ...
    #Uint32: Finjin magic number
    f.write(process_bytes(struct.pack('I', FINJIN_MAGIC), swap_bytes))
...
def unwrap_embedded_file(in_file_path, out_file_path):
    ...
    swap_bytes = False
    if magic != FINJIN_MAGIC:
        if magic == FINJIN_MAGIC_REVERSED:
            swap_bytes = True
        else:
