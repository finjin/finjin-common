...
static void PngWarningHandler(png_structp png, png_const_charp message)
{
...
PNGReader::DecompressResult PNGReader::DecompressImage(ByteBufferReader& reader, ByteBuffer& decompressedBuffer)
{
    if (reader.size_left() < 8)
        return DecompressResult::FAILED_TO_READ_SIGNATURE;
    
    if (png_sig_cmp(reader.data_left(), 0, 8) != 0)
        return DecompressResult::INVALID_SIGNATURE;

    decompressedBuffer.clear();

    //Create a temporary allocator that uses memory at the end of the specified decompressed buffer
    size_t tempAllocatorSize = 125000; 
    if (decompressedBuffer.max_size() < tempAllocatorSize)
        return DecompressResult::NOT_ENOUGH_MEMORY_TO_START_IMAGE_READ;
    ...
    ForwardAllocator<boost::null_mutex> tempAllocator;
    {
        ...
        arena.rawMemory.ptr = Allocator::AlignMemoryUp(decompressedBuffer.max_end() - tempAllocatorSize, arena.rawMemory.alignment);
        arena.rawMemory.size = decompressedBuffer.max_end() - arena.rawMemory.ptr;
    ...
    auto png = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, nullptr, PngErrorHandler, PngWarningHandler, &tempAllocator, PngMallocHandler, PngFreeHandler);
    if (png == nullptr) 
        return DecompressResult::FAILED_TO_START_IMAGE_READ;
    ...
    if (decompressedBuffer.max_size() - tempAllocatorSize < totalAllocationSize)
    {
        ...
        return DecompressResult::NOT_ENOUGH_MEMORY_FOR_IMAGE;
    }
    ...
    decompressedBuffer.resize(decompressedImageSize);
...
Utf8String PNGReader::GetDecompressResultString(DecompressResult result) const
{
    switch (result)
    {
        case DecompressResult::FAILED_TO_READ_SIGNATURE: return "Failed to read signature";
        case DecompressResult::INVALID_SIGNATURE: return "Invalid signature";
        case DecompressResult::NOT_ENOUGH_MEMORY_TO_START_IMAGE_READ: return "Not enough memory to start reading image.";
        case DecompressResult::FAILED_TO_START_IMAGE_READ: return "Failed to start reading image.";
        case DecompressResult::NOT_ENOUGH_MEMORY_FOR_IMAGE: return "Not enough memory to read image.";
        case DecompressResult::FAILED_TO_FINISH_IMAGE_READ: return "Failed to finish reading image.";
        default: return Utf8String::Empty();
    }
}

void PNGReader::DecompressImage(ByteBufferReader& reader, ByteBuffer& decompressedBuffer, Error& error)
{
    FINJIN_ERROR_METHOD_START(error);

    auto result = DecompressImage(reader, decompressedBuffer);
    if (result != DecompressResult::SUCCESS)
        FINJIN_SET_ERROR(error, GetDecompressResultString(result));
}
